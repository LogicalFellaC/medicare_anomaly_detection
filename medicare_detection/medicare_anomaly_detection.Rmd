...
As part of a project for Stanford's Data Challenge Lab, I previously explored unusual
billings by picking out key variables and creating new ones from which subsetting
criteria were created manually.

The goal of this project is to use unsuprvised anomaly detection algorithms to
expedite the process without relying on arbitrary thresholds. This would also 
have an advantage in terms of scalability.
...
To converting the raw data into a workable .rds format, an R script was
runparately.
...
This part of the section is masked from public viewing for confidentiality 
purpose.
...
First, we have to subset the data by numeric variables and those without any
missing values.

Then, I pick out only the relevant variables over which outlier scores should be 
computed.
...
What we need to do now is to get a nested dataset of `medicare_num` by 
`HCPCS_CODE` so that we can run the outlier score calculation algorithm
within each procedure / product.
...
As can be seen, there are many products / services that have less than 100
...
with much higher memory capacity).
...
+ The `k` parameter passed to `ldbod` should depend on the number of datapoints
in each of these nested datasets.
+ `NPI` should not be passed on to the `ldbod`.
+ All 4 available methods (LOF, LDF, RKOF, LPDF) are used to comput the outlier
scores (default).
+ The computed scores (a total of 9) are attached to the original nested
datasets.
